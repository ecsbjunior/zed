...
use git_pull_request::{GitPullRequestPanel, GitPullRequestViewToolbar};
...
fn initialize_panels(window: &mut Window, cx: &mut Context<Workspace>) -> Task<anyhow::Result<()>> {
    cx.spawn_in(window, async move |workspace_handle, cx| {
        ...
        let git_pull_request = GitPullRequestPanel::load(workspace_handle.clone(), cx.clone());
        let channels_panel =
        ...
        futures::join!(
            ...
            add_panel_when_ready(git_pull_request, workspace_handle.clone(), cx.clone()),
...
fn initialize_pane(
    ...
    pane.update(cx, |pane, cx| {
        pane.toolbar().update(cx, |toolbar, cx| {
            ...
            let git_pull_request_view_toolbar = cx.new(|_| GitPullRequestViewToolbar::new());
            toolbar.add_item(git_pull_request_view_toolbar, window, cx);
        })
    });
}
